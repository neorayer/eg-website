...
<div class="rep-body-box">
	<ul class="user-reputation-page-rtitle">
		<li class="user-reputation-page-left-title"><c:out value='${team.teamName}'/>的信誉:
    ...
    <div class="all-rep-box">所有评价</div>
		...
		<li class="reputation-rep-box">
			<div>
				...
				<span class="reputation-list-match"><c:out value='${reputation.bisai.title}'/></span>  
				  ...
				  <c:if test='${!reputation.recalled&&reputation.needRecall}'>
				  
					<c:if test='${ACTOR.username eq leader}'>	  
				 ...
				 <span class="reputation-list-fromuser">来自
				 	<a href="../player/index.jsplayout.vi?username=<c:out value='${reputation.fromu.username}'/>"><c:out value='${reputation.fromu.dispName}'/></a>(<c:out value='${reputation.createDate}'/>)</span> </div>
...
<script type="text/javascript">
    var Reputation={
    	backRep:function(id){
    		$(".certificate-album").load("../team/team_backrep.jsp.vi?id="+id);
    	},
    };
    $(document).ready(function(){
    	$(".reputation-pagebar a").click(function(){
    		  var url = $(this).attr('href');
			$('.certificate-album').load("../team/team_reputation.jsp.vi"+url);		
			return false;
    	});
    })
