...
<div id="allvisitor-page">
	<div id="allvisitor-page-title">
		<div class="floatLeft">
			...
			<a href="../album/photos.jsplayout.vi?id=<c:out value="${photo.albumid}"/>"><c:out value="${album.title}"/></a>
		</div>
		<div class="floatRight"><a href="../album/photos.jsplayout.vi?id=<c:out value="${photo.albumid}"/>">返回&#62;&#62;</a></div>
	</div>
	<div id="page-photos-body-box">
		<ul id="page-photos-toolbar-box">
			<li><a href="javascript:" onclick="Album.setcover('<c:out value="${photo.id}"/>','<c:out value="${photo.albumid}"/>');" >设置封面</a></li>
			<li><a href="javascript:" onclick="Album.delphoto('<c:out value="${photo.id}"/>','<c:out value="${photo.albumid}"/>');" >删除图片</a></li>
		</ul>
		...
		<div id="page-photo-list" align="center">
			...
			<div align="center" id="page-photo-avator-description-box">
				<a  id="page-modphoto-link" href="JavaScript:Album.modPhotoDesc();" title="单击此处添加照片描述">
					<c:if test='${empty photo.description}'>
						单击此处添加照片描述
					</c:if>
				...
				<form action="../album/photo.modDesc.json.do" id="page-modphoto-form" style="display: none;" method="post">
					<input type="hidden" name="id" value="<c:out value='${photo.id}'/>" />
					<textarea   name="description" id="description-textinput"><c:out value='${photo.description}' /></textarea>
					<br>
